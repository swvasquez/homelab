...
| **Gitea (SSH)** | Git clients push and pull over SSH, which is not HTTP and has no login to redirect. | Gitea's own SSH public-key authentication; keys are registered through the web UI, which stays gated. (Its **web UI** uses Model A + Model B; only the SSH port bypasses ForwardAuth.) |
